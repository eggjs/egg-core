...
class EggCore extends KoaApplication {
   ...
   */
  constructor(options = {}) {
    timing.start(`application-${process.pid}`);
  ...
  beforeStart(scope) {
    ...
    process.nextTick(() => {
      // start time should be in nextTick, and it won't be blocked by CPU when loading
      const done = this.readyCallback(name);
      timing.start('beforeStart ' + name);

      co(function* () {
        yield utils.callFn(scope);
      }).then(() => {
        done();
        timing.end('beforeStart ' + name);
      }, err => {
        done(err);
        timing.end('beforeStart ' + name);
      });
  ...
  [INIT_READY]() {
    ...
    this.ready(() => timing.end(`application-${process.pid}`));
    this.ready(() => {
      console.log(JSON.stringify(timing.toJSON(), null, 2));
    })
  }
