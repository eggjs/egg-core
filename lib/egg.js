...
const ROUTER_MAPPING = Symbol('EggCore#routerMapping');
...
class EggCore extends KoaApplication {
  ...
  /**
   * Get router instance with prefix
   * @param {String} [prefix] - Path prefix
   * @return {Router} router
   */
  getRouter(prefix = '') {
    if (this[ROUTER_MAPPING].has(prefix)) {
      return this[ROUTER_MAPPING].get(prefix);
    }
    const router = new Router({ sensitive: true, prefix }, this);
    this[ROUTER_MAPPING].set(prefix, router);
   ...
   */
  url(name, params) {
    for (const router of this[ROUTER_MAPPING].values()) {
      const result = router.url(name, params);
