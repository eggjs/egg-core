...
const CONVERTED = Symbol('egg-core#converted');
...
function convertMiddlewares(middlewares, app) {
  if (middlewares.every(middleware => middleware[CONVERTED])) {
    return middlewares;
  }
  ...
  // controller: app.get('/path', app.controller.handler)
  if (is.function(controller)) {
    wrappedController = async (ctx, next) => {
      return await utils.callFn(controller, [ ctx, next ], ctx);
    };
    wrappedController[CONVERTED] = true;
  }
  // object: app.resouce('/user', app.controller.user)
  if (controller && is.object(controller)) {
    wrappedController = {};
    for (const method in controller) {
      wrappedController[method] = async (ctx, next) => {
        return await utils.callFn(controller[method], [ ctx, next ], ctx);
      };
      wrappedController[method][CONVERTED] = true;
    }
  }
  middlewares = middlewares.concat([ wrappedController ]);
  middlewares.forEach(middleware => {
    middleware[CONVERTED] = true;
  });
  return middlewares;
}
