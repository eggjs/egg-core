...
class Router extends KoaRouter {
  ...
  patchRouterMethod() {
    ...
    methods.concat([ 'all' ]).forEach(method => {
      this[method] = (...args) => {
        const splited = spliteAndResolveRouterParams({ method, args, app: this.app });
  ...
  register(path, methods, middlewares, opts) {
    ...
    middlewares = convertMiddlewares({ app: this.app, middlewares, path, method: methods[0] });
   ...
   */
  resources(...args) {
    const splited = spliteAndResolveRouterParams({ method: 'resources', args, app: this.app });
 ...
 * @param {Object} options.method router method
 ...
 */
function spliteAndResolveRouterParams({ method, args, app }) {
  let prefix;
  let middlewares;
  let path;
  ...
  middlewares.push(resolveController({ app, controller, method, path }));
 ...
 * @param {Object} options inputs
 * @param {Object} options.app egg application instance
 * @param {String|Function} options.controller input controller
 * @param {String} options.method router method
 * @param {String} options.path router path
 * @return {Function} controller function
 */
function resolveController({ app, controller, method, path }) {
  if (is.string(controller)) {
    const actions = controller.split('.');
    let obj = app.controller;
    actions.forEach(key => {
      obj = obj[key];
      if (!obj) throw new Error(`register router(method=${method}, path=${path}) but controller '${controller}' not exists`);
    });
    controller = obj;
  }
  if (!controller) throw new Error(`register router(method=${method}, path=${path}) but controller not exists`);
  return controller;
}
 ...
 * @param {Object} options inputs
 * @param {Object} options.app egg application instance
 * @param {Array} options.middlewares middlewares and controller(last middleware)
 * @param {String} options.method router method
 * @param {String} options.path router path
 * @return {Array} middlewares
 */
function convertMiddlewares({ app, middlewares, method, path }) {
  // ensure controller is resolved
  const controller = resolveController({ app, controller: middlewares.pop(), method, path });
