...
function splitedRouterParams(args) {
  let prefix;
  let middlewares;
  if (args.length >= 3 && ((is.string(args[1]) && /^\//.test(args[1])) || is.regExp(args[1]))) {
...
function convertMiddlewares(middlewares, app) {
  ...
  middlewares = middlewares.map(middleware => {
    if (is.string(middleware)) {
      // consider this as a controller method
      middleware = toController(middleware);
    }
    return middleware;
  }).map(utils.middleware);
  ...
  function toController(controller) {
