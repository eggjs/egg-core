...
module.exports = {
  ...
  getPluginPath(plugin) {
    ...
    // support extra plugin dir for serverless environment
    // sometimes, developer will deploy node_modules to layer, when serverless function is starting up, node_modules is not the the same path as business code.
    // so we need use process.env.EGG_EXTRA_PLUGIN_DIR to add lookupDirs for loading plugin.
    // relative issue: https://github.com/serverless-components/tencent-egg/issues/11
    const extraPluginDir = process.env.EGG_EXTRA_PLUGIN_DIR;
    if (extraPluginDir) {
      lookupDirs.push(extraPluginDir);
    }
