...
const PluginManager = require('../../utils/plugin_manager');

const getPluginConfigs = Symbol('Loader#getPluginConfigs');
...
module.exports = {
  ...
  loadPlugin() {
    ...
    const appPlugins = this.appPlugins = {};
    const customPlugins = this.customPlugins = {};
    const eggPlugins = this.eggPlugins = {};

    // get `config/plugin.js` from application and framework
    for (const { config, type } of this[getPluginConfigs]()) {
      for (const name of Object.keys(config)) {
        const pluginConfig = config[name];
        let plugin = this.allPlugins[name];
        if (!plugin) {
          plugin = new Plugin(name, config[name]);
          // set plugin map
          switch (type) {
            case 'app':
              this.appPlugins[name] = plugin;
              break;
            case 'egg':
              this.eggPlugins[name] = plugin;
              break;
            case 'custom':
              this.customPlugins[name] = plugin;
              break;
            default:
          }
          this.allPlugins[name] = plugin;
        }
        plugin.mergeConfig(pluginConfig, configPath);
      }
    }

    const manager = new PluginManager(this.allPlugins);
    manager.getOrder();
  },
  ...
  [getPluginConfigs]() {
    ...
    const names = [ 'config/plugin.default.js' ];
    if (this.serverEnv !== 'default') {
      names.push(`config/plugin.${this.serverEnv}.js`);
    }

    // load from framework to application
    const unitPaths = this.eggPaths.concat([ this.options.baseDir ]);

    for (const name of names) {
      for (const unitPath of unitPaths) {
        let configPath = path.join(unitPath, name);
        ...
        configs.push({
          ...
          type: this.options.baseDir === configPath ? 'app' : 'egg',
        });
      }
    }
    ...
    if (process.env.EGG_PLUGINS) {
      try {
        configs.push({
          config: JSON.parse(process.env.EGG_PLUGINS),
          type: 'custom',
        });
      } catch (e) {
