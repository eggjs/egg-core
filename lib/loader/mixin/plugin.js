...
module.exports = {
  ...
  getPluginPath(plugin) {
    ...
    for (let i = this.eggPaths.length - 1; i >= 0; i--) {
      ...
      let pnpmPackagePath = path.dirname(eggPath);
      if (path.basename(pnpmPackagePath).startsWith('@')) {
        // 'node_modules/.pnpm/@zjy+egg-pnpm-bug@0.0.1/node_modules/@zjy/egg-pnpm-bug/node_modules',
        // 'node_modules/.pnpm/@zjy+egg-pnpm-bug@0.0.1/node_modules',  <- this is the sibling directory
        pnpmPackagePath = path.dirname(pnpmPackagePath);
      }
      lookupDirs.add(pnpmPackagePath);
    }
