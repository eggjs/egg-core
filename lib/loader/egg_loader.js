...
class EggLoader {
  ...
  getEggPaths() {
    ...
    // should find the $root/node_modules under npm3, as $root maybe not $cwd
    eggPaths.find(realpath => {
      const suffix = '/node_modules/egg';
      if (realpath.substr(-suffix.length) === suffix) {
        const rootPath = realpath.substr(0, realpath.length - suffix.length);
        if (eggPaths.indexOf(rootPath) === -1) {
          eggPaths.unshift(rootPath);
        }
        return true;
      }
      return false;
