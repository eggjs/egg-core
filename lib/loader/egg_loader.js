...
class EggLoader {
  ...
  loadToApp(directory, property, opt) {
    ...
    timing.start(`loadToApp ctx.${property}`);
    new FileLoader(opt).load();
    timing.end(`loadToApp ctx.${property}`);
  }
  ...
  loadToContext(directory, property, opt) {
    ...
    timing.start(`loadToContext ctx.${property}`);
    new ContextLoader(opt).load();
    timing.end(`loadToContext ctx.${property}`);
  }
