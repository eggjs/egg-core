...
class EggLoader {
  ...
  getEggPaths() {
    ...
    while (proto) {
      ...
      assert(fs.existsSync(eggPath), `eggPath(${eggPath}) should exists`);
  ...
  getLoadUnits() {
    ...
    for (const eggPath of this.eggPaths) {
      const pkgPath = path.join(eggPath, 'package.json');
      try {
        const frameworkInfo = require(pkgPath);
        dirs.push({
          path: eggPath,
          type: 'framework',
          name: frameworkInfo && frameworkInfo.name,
        });
      } catch (err) {
        console.warn(`${pkgPath} not exists`);
      }
    }
