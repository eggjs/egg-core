...
class FileLoader {
   ...
   * @param {String} options.leafCaseStyle - set leaf property's case when converting a filepath to property list, if not provided, default as `options.caseStyle`.
   */
  constructor(options) {
  ...
  parse() {
    ...
    for (const directory of directories) {
      ...
      for (const filepath of filepaths) {
        ...
        if (exports == null || (filter && filter(exports, { pathName, fullpath }) === false)) continue;
...
// a/b/c.js => ['a', 'b', 'c']
function getProperties(filepath, options) {
  ...
  return defaultCamelize(filepath, options);
}
...
function defaultCamelize(filepath, options) {
  ...
  const result = properties.map(property => {
    ...
    return changeCase(property, options.caseStyle);
  });

  if (options.leafCaseStyle) {
    const lastIndex = properties.length - 1;
    result[ lastIndex ] = changeCase(properties[lastIndex], options.leafCaseStyle);
  }
  return result;
}

function changeCase(property, caseStyle) {
