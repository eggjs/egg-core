...
class FileLoader {
   ...
   * @param {String} options.extensions - match the file extension when load, default to .js and .ts
  ...
  constructor(options) {
    ...
    const extensions = this.options.extensions.map(ext => ext.substring(1));
    this.defaultMatch = [ '**/*.(' + extensions.join('|') + ')' ];
    if (extensions.includes('ts')) {
      this.defaultMatch.push('!**/*.d.ts');
    }
